...
import { getLatestReviewPayload, getReviewPayloadForRun } from "./review-payload-service.js";
...
export async function handleReviewPayloadApiRequest(request, response, { store }) {
  ...
  const latestMatch = runMatch ? null : latestReviewPattern.exec(url.pathname);
  if (!runMatch && !latestMatch) return false;
  ...
  try {
    const slug = decodeURIComponent((runMatch || latestMatch)[1]);
    const payload = runMatch
