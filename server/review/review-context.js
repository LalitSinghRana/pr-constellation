...
export async function loadReviewContext(
  ...
  const fallbackUrl = `https://github.com/${parsed.owner}/${parsed.repo}/pull/${parsed.number}`;
  const metadata = await resolvePresentation(parsed, fallbackUrl, {
    fetchMetadata,
    loadInboxMetadata,
  });
  const coordinates = pullRequestCoordinatesFromMetadata(metadata, parsed);
...
function pullRequestCoordinatesFromMetadata(metadata, fallback) {
  const url = typeof metadata?.url === "string" && metadata.url ? metadata.url : "";
  if (url) {
  ...
  return {
    ...
    prUrl: `https://github.com/${fallback.owner}/${fallback.repo}/pull/${fallback.number}`,
    repo: fallback.repo,
  };
}
