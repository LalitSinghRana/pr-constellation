...
test("done items stay in the done view after activity updates change version", async (context) => {
  const root = await mkdtemp(path.join(os.tmpdir(), "prc-inbox-done-version-"));
  context.after(() => rm(root, { recursive: true, force: true }));
  const store = await createInboxStore({ databasePath: path.join(root, "cockpit.sqlite3") });
  context.after(() => store.close());
  await store.mutateQueueState((state) =>
    Object.assign(
      ...
      queueState({
        merged: {
          ...doneRecord("merged", "2026-01-01T00:00:00.000Z"),
          item: {
            id: "merged",
  ...
  await store.mutateQueueState((state) => {
    state.items.merged.version = "2026-01-05T00:00:00.000Z";
    state.items.merged.item.updatedAt = "2026-01-05T00:00:00.000Z";
  });

  assert.deepEqual(Object.keys(store.readQueueState({ view: "active" }).items), []);
  assert.deepEqual(Object.keys(store.readQueueState({ view: "done" }).items), ["merged"]);
  assert.deepEqual(store.queueCounts(), { active: 0, done: 1, total: 1 });
});
