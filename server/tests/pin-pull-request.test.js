...
import { prFromActivity } from "../inbox/inbox-service/activity.js";
...
test("pinning uses GitHub repository casing from activity", async () => {
  const result = await addPinnedInboxPullRequest(
    "https://github.com/picnicsupermarket/picnic-store-config/pull/5230",
    {
      getActivity: async () => ({
        author: { login: "alice" },
        createdAt: "2026-08-01T00:00:00.000Z",
        ...
        title: "Config",
        updatedAt: "2026-08-01T00:00:00.000Z",
        url: "https://github.com/PicnicSupermarket/picnic-store-config/pull/5230",
      }),
      mutateQueueState: async (callback) => callback({ items: {} }),
      now: new Date("2026-08-20T12:00:00.000Z"),
    },
  ...
  assert.equal(result.id, "PicnicSupermarket/picnic-store-config#5230");
  assert.equal(result.repository, "PicnicSupermarket/picnic-store-config");
});

test("activity repository name comes from GitHub not the inbox seed", () => {
  const item = {
    number: 5230,
  ...
  assert.equal(pr.repository.nameWithOwner, "PicnicSupermarket/picnic-store-config");
  assert.equal(pr.url, "https://github.com/PicnicSupermarket/picnic-store-config/pull/5230");
});
