...
test("queue all analyses skip authored, done, merged, and closed pull requests", async () => {
  const queued = [];
...
test("PR and non-PR notification threads stay visible", () => {
  ...
  assert.equal(prFromNotification(pullRequest).notificationThreadId, "NT_123");
  assert.equal(otherNotificationFromThread(issue).url, "https://github.com/example/repo/issues/7");
  assert.equal(otherNotificationFromThread(issue).notificationThreadId, "NT_123");
});
...
test("non-PR notifications survive the local queue round trip", () => {
  ...
  assert.equal(restored.id, "notification:NT_123");
...
test("GraphQL sync remaps leftover numeric notification queue ids", () => {
  const state = { version: 2, sync: {}, items: {} };
  rememberQueueItems(
    state,
    [
  ...
  setQueueItemDone(state, "notification:123", true);
  remapNotificationQueueIds(state, [
    otherNotificationFromThread({
      databaseId: 123,
      id: "NT_kwDOExample",
      reason: "mention",
  ...
  assert.equal(state.items["notification:123"], undefined);
  const [restored] = inboxFromQueue(state).notifications;
  assert.equal(restored.id, "notification:NT_kwDOExample");
  assert.equal(restored.notificationThreadId, "NT_kwDOExample");
...
test("inbox notifications seed any repository including watch-subscribed pull requests", () => {
  ...
  assert.equal(reviewRequested.get("other-org/other-repo#3541").notificationThreadId, "NT_456");
...
test("local read and Done state reopen for PR updates", () => {
  ...
  assert.equal(typeof state.items[item.id].doneAt, "string");
  assert.equal(applyQueueState([item], state)[0].doneAt, state.items[item.id].doneAt);
  ...
  assert.equal(reopened.done, true);
  ...
  assert.equal(state.items[item.id].doneAt, undefined);
...
test("tracked PR snapshots restore local membership and migrate old records", () => {
  ...
  assert.equal(restored.notificationThreadId, "NT_123");
  assert.deepEqual(
...
test("inbox membership keeps locally done items when GitHub thread is unchanged", () => {
  const items = new Map();
  addSource(items, pr, "repository");
  const entries = rankItems(items);
  const state = { version: 2, sync: {}, items: {} };
  rememberQueueItems(state, entries, "2026-07-31T12:00:00Z");
  setQueueItemDone(state, "example/repo#42", true, new Date("2026-08-01T12:00:00Z"));

  const thread = { updated_at: "2026-07-31T12:00:00Z" };
  const notifications = [{ pr, thread }];
  applyInboxMembership(state, inboxIdsFromNotifications(notifications), [], {
    notificationUpdatedAtById: notificationUpdatedAtByIdFromNotifications(notifications),
  });

  assert.equal(applyQueueState(entries, state).find((item) => item.number === 42).done, true);
});

test("pinned inbox pull requests stay active until GitHub notifies", () => {
  const items = new Map();
  addSource(items, pr, "repository");
  addSource(
    items,
  ...
  pinQueueItem(
    state,
    entries.find((item) => item.number === 43),
  ...
  assert.equal(state.items["example/repo#43"].pinned, true);

  applyInboxMembership(
    state,
    inboxIdsFromNotifications([
      { pr },
      { pr: { ...pr, number: 43, url: "https://github.com/example/repo/pull/43" } },
    ]),
  );
  assert.equal(state.items["example/repo#43"].pinned, undefined);
  assert.equal(applyQueueState(entries, state).find((item) => item.number === 43).done, false);
});

test("marking a pinned inbox item done clears the pin", () => {
  const items = new Map();
  addSource(items, pr, "repository");
  const [item] = rankItems(items);
  const state = { version: 2, sync: {}, items: {} };
  pinQueueItem(state, item, "2026-07-31T12:00:00Z");
  setQueueItemDone(state, item.id, true);
  assert.equal(state.items[item.id].pinned, undefined);
  assert.equal(applyQueueState([item], state)[0].done, true);
});
...
test("authored pull requests stay in My PRs and look read without new notifications", () => {
  ...
  setQueueItemDone(state, item.id, true, new Date("2026-07-31T12:00:00Z"));

  applyAuthoredReadState(state, [item.id], []);
  const archived = applyQueueState([item], state)[0];
