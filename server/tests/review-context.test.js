...
test("review context uses GitHub owner casing from metadata url when slug is lowercase", async (context) => {
  const reviewRoot = await mkdtemp(path.join(os.tmpdir(), "prc-review-context-casing-"));
  context.after(() => rm(reviewRoot, { force: true, recursive: true }));
  const slug = "gh-17-picnicsupermarket-19-picnic-store-config-5230";
  const review = await loadReviewContext(slug, {
    fetchMetadata: async () => ({
      author: { login: "alice" },
      headRefOid: "abc123",
  ...
  assert.equal(review.owner, "PicnicSupermarket");
  assert.equal(review.repo, "picnic-store-config");
  assert.equal(review.prUrl, "https://github.com/PicnicSupermarket/picnic-store-config/pull/5230");
  assert.equal(review.runId, null);
});
