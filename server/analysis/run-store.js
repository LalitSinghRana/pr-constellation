...
export class RunStore {
  ...
  async removeSlugDir(slug) {
    assertStorageId(slug, "slug");
    const slugDir = path.join(this.#reviewsDir, slug);
    const reviewsRealPath = await ensureRealDirectory(this.#reviewsDir);
    let stats;
    try {
      stats = await lstat(slugDir);
    } catch (error) {
      if (error?.code === "ENOENT") {
        return false;
      }
    ...
    if (!stats.isDirectory()) {
      throw createStoreError("INVALID_RUN_DOCUMENT", `Slug "${slug}" is not a directory.`);
    }
    const slugRealPath = await realpath(slugDir);
    assertPathContained(reviewsRealPath, slugRealPath, "slug");
