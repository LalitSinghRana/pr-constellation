...
export function prFromActivity(item, activity) {
  ...
  return {
    ...
    url: activity.url,
    repository: { nameWithOwner: resolveRepositoryNameWithOwner(activity, item) },
...
function resolveRepositoryNameWithOwner(activity, item) {
  const fromQuery = activity.repository?.nameWithOwner;
  if (typeof fromQuery === "string" && fromQuery.trim()) {
    return fromQuery.trim();
  }
  if (typeof activity.url === "string" && activity.url) {
    try {
      const parsed = parseGitHubPrUrl(activity.url);
      return `${parsed.owner}/${parsed.repo}`;
    } catch {
      // Fall back to the inbox seed when GitHub did not return a repository name.
    }
  }
  return repositoryName(item);
}
...
export async function getPrActivity(pr) {
  ...
  const activity = repository?.pullRequest;
  if (!activity) throw new Error("Pull request activity unavailable");
  return {
    ...activity,
    repository: {
      nameWithOwner: typeof repository?.nameWithOwner === "string" ? repository.nameWithOwner : "",
    },
  };
}
