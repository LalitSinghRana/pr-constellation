...
export function normalizeQueueState(value = {}) {
  ...
  for (const [id, record] of Object.entries(value.items ?? {}).slice(0, 20_000)) {
    ...
    items[id] = {
      ...
      ...(typeof record.doneAt === "string" ? { doneAt: record.doneAt } : {}),
...
export function queueItemFromRecord(id, record) {
  ...
  try {
    ...
    const notificationThreadId = validNotificationThreadId(notificationMatch?.[1]);
    if (notificationThreadId) {
      ...
      return {
        ...
        ...(Number.isInteger(stored.databaseId) ? { databaseId: stored.databaseId } : {}),
      };
    }
    ...
    return {
      ...
      closedAt: typeof stored.closedAt === "string" ? stored.closedAt : null,
...
export function remapNotificationQueueIds(state, otherNotifications = []) {
  for (const item of otherNotifications) {
    const nextId = typeof item?.id === "string" ? item.id : "";
    const nextThreadId = validNotificationThreadId(item?.notificationThreadId);
    if (!nextId.startsWith("notification:") || !nextThreadId) continue;

    const oldId = findLegacyNotificationQueueId(state, item, nextId);
    if (!oldId) continue;

    const previous = state.items[oldId];
    if (!state.items[nextId]) {
      state.items[nextId] = {
        ...previous,
        item: {
          ...previous.item,
          id: nextId,
          notificationThreadId: nextThreadId,
        },
      };
    }
...
function findLegacyNotificationQueueId(state, item, nextId) {
  const legacyId = Number.isInteger(item.databaseId) ? `notification:${item.databaseId}` : "";
  if (legacyId && legacyId !== nextId && state.items[legacyId]) return legacyId;
  const url = typeof item.url === "string" ? item.url : "";
  if (!url) return "";
  return (
    Object.keys(state.items).find((id) => {
      if (id === nextId || !id.startsWith("notification:")) return false;
      const record = state.items[id];
      return record.item?.url === url || record.url === url;
    }) ?? ""
  );
}
...
export function applyQueueState(entries, state) {
  return entries.map((item) => {
    ...
    const notificationAt = Date.parse(
      item.notificationUpdatedAt || record.notificationUpdatedAt || "",
    );
    const doneAt = Date.parse(record.doneAt || "");
    const resurfacedByNotification =
      Boolean(doneVersion) &&
      Number.isFinite(doneAt) &&
      Number.isFinite(notificationAt) &&
      notificationAt > doneAt;
    return {
      ...item,
      queueVersion: version,
      done: Boolean(doneVersion) && !resurfacedByNotification,
      doneAt: typeof record.doneAt === "string" ? record.doneAt : null,
...
export function setQueueItemDone(state, id, done, now = new Date()) {
  const record = state.items[id];
  ...
  if (done) {
    ...
    record.doneAt = now instanceof Date ? now.toISOString() : String(now);
...
export function notificationUpdatedAtByIdFromNotifications(
  ...
  for (const { thread, pr } of pullRequestNotifications) {
    ...
    if (!Number.isFinite(previous) || at > previous) times.set(id, at);
  }
  for (const item of otherNotifications) {
    if (!item?.id) continue;
    const at = Date.parse(item.notificationUpdatedAt ?? item.updatedAt ?? "");
    if (Number.isFinite(at)) times.set(item.id, at);
  }
  return times;
}

export function applyInboxMembership(
  state,
  inboxIds,
  authoredOpenIds = [],
  { notificationUpdatedAtById = null } = {},
) {
  ...
  const notificationAt =
    notificationUpdatedAtById instanceof Map ? notificationUpdatedAtById : null;
  for (const id of Object.keys(state.items)) {
    const record = state.items[id];
    const markedDone = Boolean(record?.doneVersion);
    if (present?.has(id)) {
      if (record.pinned) delete record.pinned;
      if (markedDone) {
        const threadAt = notificationAt?.get(id);
        const doneAt = record.doneAt ? new Date(record.doneAt).getTime() : 0;
        if (threadAt == null || !doneAt || threadAt > doneAt) {
          setQueueItemDone(state, id, false);
        }
      }
      continue;
    }
    ...
    if (present && record?.version && !markedDone) {
    ...
    if (
      ...
      record?.version &&
      !markedDone &&
      isOpenAuthoredPullRequest(record.item)
    ) {
      setQueueItemDone(state, id, true);
    }
  }
...
export function pinQueueItem(state, item, seenAt) {
  rememberQueueItems(state, [item], seenAt);
  const record = state.items[item.id];
  if (!record) return null;
  if (record.doneVersion) {
    setQueueItemDone(state, item.id, false);
  }
  record.pinned = true;
  return { id: item.id, pinned: true };
}
...
export function applyAuthoredReadState(state, authoredOpenIds, authoredNotificationIds = null) {
  ...
  for (const id of authored) {
    ...
    if (record.doneVersion) {
      if (notified?.has(id)) {
        const notificationAt = Date.parse(record.notificationUpdatedAt || "");
        const doneAt = Date.parse(record.doneAt || "");
        if (Number.isFinite(notificationAt) && Number.isFinite(doneAt) && notificationAt > doneAt) {
          setQueueItemDone(state, id, false);
        }
      }
      continue;
