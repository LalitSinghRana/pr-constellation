...
import { handleGitHubMediaRequest } from "../review/github-media.js";
...
async function assertServerPortAvailable() {
  ...
  await new Promise((resolve, reject) => {
    ...
    throw new Error(
      `Port ${port} is already in use; stop the existing PR Constellation server first.`,
      {
        cause: error,
      },
    );
...
export async function startServer({
  ...
  server.on("request", (request, response) => {
    ...
    handleApiRequest(request, response, { dashboardService, eventHub, scheduler })
      .then(async (handled) => {
        if (handled) return;
        if (await handleGitHubMediaRequest(request, response)) return;
  ...
  console.log(`PR Constellation: ${cockpitOrigin}/`);
