...
export function createReviewSlug({ owner, repo, number }) {
...
export function parseReviewSlug(slug) {
  if (typeof slug !== "string" || !slug.startsWith("gh-")) {
    throw new Error(`Expected a review slug, got: ${slug}`);
  }
  const body = slug.slice(3);
  const ownerLenMatch = /^(\d+)-/.exec(body);
  if (!ownerLenMatch) {
    throw new Error(`Expected a review slug, got: ${slug}`);
  }
  const ownerLen = Number(ownerLenMatch[1]);
  const afterOwnerLen = body.slice(ownerLenMatch[0].length);
  const owner = afterOwnerLen.slice(0, ownerLen);
  if (owner.length !== ownerLen || afterOwnerLen[ownerLen] !== "-") {
    throw new Error(`Expected a review slug, got: ${slug}`);
  }
  const afterOwner = afterOwnerLen.slice(ownerLen + 1);
  const repoLenMatch = /^(\d+)-/.exec(afterOwner);
  if (!repoLenMatch) {
    throw new Error(`Expected a review slug, got: ${slug}`);
  }
  const repoLen = Number(repoLenMatch[1]);
  const afterRepoLen = afterOwner.slice(repoLenMatch[0].length);
  const repo = afterRepoLen.slice(0, repoLen);
  if (repo.length !== repoLen || afterRepoLen[repoLen] !== "-") {
    throw new Error(`Expected a review slug, got: ${slug}`);
  }
  const number = afterRepoLen.slice(repoLen + 1);
