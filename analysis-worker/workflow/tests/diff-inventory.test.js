import {
  createDiffInventory,
  createDiffSummary,
} from "../03-build-diff-inventory/diff-inventory.js";
...
assertEqual(
  inventory.changedLines.map((line) => line.kind).join(","),
  "delete,insert,insert",
  "changed line kinds",
);
assertEqual(
  inventory.changedLines.map((line) => line.id).join(","),
  "file-1:hunk-1:line-2,file-1:hunk-1:line-3,file-1:hunk-1:line-4",
  "changed line ids",
);
...
assertEqual(
  summary.files[0].hunks[0].lines.map((line) => line.id).join(","),
  inventory.changedLines.map((line) => line.id).join(","),
  "summary changed line ids",
);
