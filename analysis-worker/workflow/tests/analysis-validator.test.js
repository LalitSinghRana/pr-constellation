  ...
  reviewStacks: [
    {
      id: "runtime",
      title: "Runtime value and expectation",
      explanation: "The assertion must follow the runtime behavior it verifies.",
      fileIds: [runtimeFile.id],
    },
    {
      id: "tests",
      title: "Test verification",
      explanation: "Failed expectations would hide regressions in the runtime value change.",
      fileIds: [testFile.id],
    },
  ],
};
...
function buildFile({
  ...
  return {
    ...
    reviewPriority,
    changeKind,
    explanation: `The runtime contract in ${file.path} must stay consistent with the stack outcome.`,
...
function buildSection({
  ...
  return {
    ...
    changeKind,
    explanation:
      "If this step is wrong, callers may read stale values or skip validation entirely.",
...
function branch(parentId, childId, order = 0) {
  return {
    ...
    childId,
    order,
    explanation: `${childId} supports ${parentId} by proving the dependent behavior still holds.`,
  };
}
