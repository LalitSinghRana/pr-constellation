...
const compactProbeSchema = JSON.stringify(sanitizeClaudeSchema(CLAUDE_PROBE_SCHEMA));
...
assert.deepEqual(probeArgs.slice(probeArgs.indexOf("--model"), probeArgs.indexOf("--model") + 2), [
  "--model",
  "claude-sonnet-4-6",
]);
assert.deepEqual(
  probeArgs.slice(probeArgs.indexOf("--effort"), probeArgs.indexOf("--effort") + 2),
  ["--effort", "high"],
);
assert.equal(probeArgs.at(probeArgs.indexOf("--json-schema") + 1), compactProbeSchema);
