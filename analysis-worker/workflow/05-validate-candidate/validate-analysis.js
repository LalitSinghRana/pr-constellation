...
import { appendBriefingTextErrors, appendTitleTextErrors } from "./briefing-checks.js";
...
export function validateReviewAnalysis(analysis, { inventory = null } = {}) {
  ...
  if (!isNonEmptyString(analysis?.intent)) {
    ...
    appendBriefingTextErrors(errors, analysis.intent, {
  ...
  if (!isNonEmptyString(analysis?.summary)) {
    ...
    appendBriefingTextErrors(errors, analysis.summary, {
      label: "analysis.json summary",
    });
  }
  if (
...
function validateFiles({ analysis, errors, inventory }) {
  ...
  const hunkLineLocations = indexHunkLineLocations(inventory);
  ...
  for (const file of analysis.files) {
    ...
    appendBriefingTextErrors(errors, file.explanation, {
      label: `file ${file.id} explanation`,
    });
...
function validateSectionTree({
  ...
  for (const section of file.sectionTree.sections) {
    ...
    appendTitleTextErrors(errors, section.title, {
      label: `review section ${owner} title`,
    });
    ...
    appendBriefingTextErrors(errors, section.explanation, {
      label: `review section ${owner} explanation`,
      title: section.title,
    });
...
function validateReviewStackEntries({
  ...
  for (const [index, stack] of reviewStacks.entries()) {
    ...
    appendTitleTextErrors(errors, stack?.title, {
      label: `${stackLabel} title`,
    });
    validateRequiredText({ errors, label: `${stackLabel} explanation`, value: stack?.explanation });
    appendBriefingTextErrors(errors, stack?.explanation, {
      label: `${stackLabel} explanation`,
      title: stack?.title,
      stackBriefing: true,
    });
...
function validateTreeBranches({
  ...
  for (const [index, branch] of branches.entries()) {
    ...
    appendBriefingTextErrors(errors, branch.explanation, {
      label: `${branchLabel} ${branchId} explanation`,
    });
...
function validateChangedLineSequence({
  ...
  if (Array.isArray(section.changedLineRanges)) {
    validateSectionChangedLineRanges({
      changedLineIds,
      errors,
      file,
      inventoryFile,
      locations: hunkLineLocations,
      owner,
      section,
    });
    return;
