...
Follow the shared **mini-node-first ladder** for roots, sibling `order`, and
section `reviewPriority`. Keep these structural rules:
...
- Every tree branch explanation follows the shared briefing contract for
  mini-edges: start with the consequence, explain why the child belongs under
  the parent. Put any rare explicit check under a final `Reviewer attention:`
  section.
...
- Describe internal variants and details in the section title rather than
  extracting them from their cohesive section solely to create more concepts.
  Section titles name the chunk; they do not carry the briefing. Section
  explanations follow the shared briefing contract for mini-nodes.
...
- `skim/imports`: "Bring in hooks and Pressable symbols"
- `skim/type`: "Shape the secondary content variant"
...
Follow the shared **file-first ladder** for roots, sibling `order`, and file
`reviewPriority`. Keep these structural rules:
...
- the overall review `intent`, `summary`, and `confidence` (follow the shared
  intent/summary guidance)
