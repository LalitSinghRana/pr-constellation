...
export async function runCli(args) {
  ...
  if (options.command === "view") {
    throw new Error(
      "prc view is no longer supported. Open http://127.0.0.1:4397/reviews/<slug>/ in PR Constellation.",
    );
  }
...
function parseArgs(args) {
  ...
  for (const arg of args) {
    if (arg === "--") {
      ...
      // Legacy flags ignored; review UI is served by the PR Constellation SPA.
