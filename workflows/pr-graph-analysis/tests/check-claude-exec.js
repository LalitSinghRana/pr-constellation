...
assert.equal(
  args.at(args.indexOf("--mcp-config") + 1),
  '{"mcpServers":{}}',
);
...
assert.equal(
  args.at(args.indexOf("--json-schema") + 1),
  compactClaudeSchema,
);
assert.equal(schema.properties.sections.minItems, 1);
assert.equal(
  "minItems" in JSON.parse(compactClaudeSchema).properties.sections,
  false,
);
...
const temporaryRoot = await mkdtemp(
  path.join(os.tmpdir(), "prc-claude-exec-"),
);
...
const originalDescendantPidPath =
...
try {
  ...
  assert.equal(
    ...
    "Analyze this frozen pull request.",
  );
  ...
  assert.equal(
    invokedArgs.at(invokedArgs.indexOf("--json-schema") + 1),
    compactClaudeSchema,
  );
  ...
  await assert.rejects(
    ...
    (error) => (
      /without a result event/.test(error?.message)
  ...
  restoreEnvironmentVariable(
    "PRC_TEST_CLAUDE_ARGS_PATH",
    originalArgsPath,
  );
  restoreEnvironmentVariable(
    "PRC_TEST_CLAUDE_PROMPT_PATH",
    originalPromptPath,
  );
  restoreEnvironmentVariable("PRC_TEST_CLAUDE_PID_PATH", originalPidPath);
  restoreEnvironmentVariable(
    "PRC_TEST_CLAUDE_DESCENDANT_PID_PATH",
    originalDescendantPidPath,
  );
