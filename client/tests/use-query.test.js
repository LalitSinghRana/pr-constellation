...
test("useQuery distinguishes isLoading from background isPending", async () => {
  const first = deferred();
  const second = deferred();
  let calls = 0;
  let latest;
...
test("useQuery refetch returns fetched data", async () => {
  let calls = 0;
  let latest;
  ...
  try {
    ...
    await act(async () => refetchDone);
    assert.deepEqual(await refetchDone, { n: 2 });
  } finally {
    await act(async () => root.unmount());
  }
});
