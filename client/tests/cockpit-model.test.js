...
test("inbox destination maps a pull request to its sidebar filter and repository tab", () => {
  assert.deepEqual(
    inboxDestination({
      authored: false,
      lifecycle: "new",
      repository: "acme/example-modules",
      state: "OPEN",
    }),
    {
      filter: "new",
  ...
  assert.deepEqual(
    ...
    { filter: "mine", repository: "example/app" },
  );
  assert.equal(repositoryTabLabel("acme/example-modules"), "Example Modules");
  assert.deepEqual(inboxDestination({ url: "https://github.com/example/app/pull/9" }), {
    filter: "new",
    repository: "example/app",
  });
  assert.deepEqual(
    ...
    { filter: "done", repository: "example/app" },
  );
  assert.equal(
    inboxJumpHref({
      authored: false,
    ...
    "/?project=acme/example-modules&filter=merged",
  );
});
