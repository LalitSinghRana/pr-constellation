...
function ReviewSection({ children, className, ...props }) {
  return (
    <article className={cn("review-section-node", className)} {...props}>
      {children}
    </article>
  );
}
...
function ReviewSectionNode({ data }) {
  ...
  return (
    <ReviewSection
      ...
      className={cn(
        "nodrag nopan w-full max-w-full cursor-text overflow-hidden rounded-md border border-border bg-card shadow-sm select-text",
        reviewPriority === "primary" &&
