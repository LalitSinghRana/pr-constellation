...
import { relativeTime } from "../lib/queue.js";
  ...
  return (
    ...
    >
      {timeline.length === 0 ? (
        ...
        <Timeline
          className="flex w-full min-w-0 max-w-5xl flex-col"
          ...
          pt={{
            connector: { className: "w-px flex-1 bg-border" },
            content: { className: "min-w-0" },
            event: {
              className:
                "grid min-w-0 grid-cols-[1.5rem_minmax(0,1fr)] gap-3 not-last:[&>[data-pc-section=content]]:pb-3",
            },
            opposite: { className: "hidden" },
            separator: { className: "flex min-h-full flex-col items-center" },
          }}
          unstyled
          value={timeline}
        />
...
function ConversationItem({ item }) {
  ...
  const timestamp = <ConversationTimestamp createdAt={item.createdAt} />;
  return (
    <article
      className={cn(
    ...
    >
      <p
        className={cn(
          "flex min-w-0 items-center gap-1.5 font-semibold text-muted-foreground",
          isContent ? "text-xs" : "",
        )}
      >
        <span className={cn("min-w-0", isContent ? "" : "truncate")}>
          {item.actor} {label}
        </span>
        {timestamp ? (
          <>
      ...
      {isContent && item.body ? <GitHubMarkdown body={item.body} className="mt-3" /> : null}
    </article>
  );
}
  ...
  return (
    <div className="mt-3 grid gap-3">
      ...
      {thread.comments.map((comment) => {
        const timestamp = <ConversationTimestamp createdAt={comment.createdAt} />;
        return (
          <div className="border-t pt-3" key={comment.id || comment.createdAt}>
            ...
            <GitHubMarkdown body={comment.body} className="mt-3" />
          </div>
        );
      })}
    </div>
...
function ConversationTimestamp({ createdAt }) {
