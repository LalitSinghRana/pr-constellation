...
export const REVIEW_CAMERA_PADDING_X = 80;
export const REVIEW_STEP_MAX_ZOOM = 1.25;
...
export function buildReviewTree(
  analysis,
  {
    ...
    sectionMaxHeight = Number.POSITIVE_INFINITY,
    foldGroups = true,
    ...
    for (const item of layout.sections) {
      nodes.push({
        ...
        style: {
          height: item.height,
          pointerEvents: "auto",
          width: reviewSectionWidth(item.section),
        },
...
function buildFileLayout(
  ...
  const layout = layoutReviewSections(reviewSections, sectionTree.branches, sectionMaxHeight);
...
function layoutReviewSections(
  ...
  const items = reviewSections.map((section, order) => ({
    height: reviewSectionHeight(section, sectionMaxHeight),
    section,
    order,
...
export function reviewSectionHeight(reviewSection, sectionMaxHeight = Number.POSITIVE_INFINITY) {
  ...
  const estimated = Math.max(120, REVIEW_SECTION_HEADER_HEIGHT + lineCount * 18 + 2);
  return Math.min(estimated, sectionMaxHeight);
}

export function sectionMaxHeightFromCanvas(canvasHeight) {
  if (!canvasHeight || canvasHeight <= 0) {
    return Number.POSITIVE_INFINITY;
  }

  const visibleHeight = canvasHeight - VIEWPORT_PADDING_Y - FILE_NODE_PADDING;
  return Math.max(120, Math.floor(visibleHeight / REVIEW_STEP_MAX_ZOOM));
}
