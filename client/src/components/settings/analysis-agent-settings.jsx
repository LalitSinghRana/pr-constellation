...
export function useAnalysisCatalog() {
  const [catalog, setCatalog] = useState(() => createAnalysisCatalog());

  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/analysis-models", { signal: controller.signal })
      .then((response) => (response.ok ? response.json() : null))
      .then((payload) => {
        if (payload?.providers?.length) setCatalog(payload);
      })
      .catch(() => {});
