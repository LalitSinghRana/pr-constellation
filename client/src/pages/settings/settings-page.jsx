...
import { AnalysisAgentSettings, useAnalysisCatalog } from "./analysis-agent-settings.jsx";
import { ScoringCard } from "./scoring-card.jsx";
import { TeamSettingsForm } from "./team-settings-form.jsx";
...
export function SettingsPage() {
  useDocumentTitle({ title: "Settings · PR Constellation" });
      ...
      <div className="mx-auto w-full max-w-[1240px] px-5 pt-12 pb-20 sm:px-8 max-[700px]:px-4 max-[700px]:pt-6">
        ...
        <header className="mt-10">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">
            Turn features on or off, configure the team used to score the inbox, and inspect the
            priority model. Preferences are saved in the local SQLite settings store.
          </p>
        </header>
          ...
          <ItemGroup className="gap-0 overflow-hidden rounded-lg border bg-card/82 shadow-lg backdrop-blur">
                ...
                <div className="flex w-full items-center gap-4 px-5 py-4">
                  <ItemContent>
                    ...
                    <ItemDescription className="line-clamp-none">
                      Teammates and teams used to score the inbox.
                    </ItemDescription>
                  </ItemContent>
                  <ItemActions>
            ...
            <Item size="sm" className="items-start rounded-none border-0 px-5 py-4 max-sm:flex-col">
              <ItemContent>
                ...
                <ItemDescription className="line-clamp-none">
                  Files open as the section mini-tree unless you prefer the full file (source-order)
                  view.
                </ItemDescription>
              </ItemContent>
              <ItemActions>
                <Tabs
                  onValueChange={(defaultFileViewMode) => patchSettings({ defaultFileViewMode })}
                  value={settings.defaultFileViewMode}
                >
                  <TabsList aria-label="Default file view">
                    {FILE_VIEW_MODES.map((mode) => (
                      <TabsTrigger className="capitalize" disabled={busy} key={mode} value={mode}>
